...
describe('specify-explicit-webresource-name', function () {
    ...
    it('should auto-generate the name if there is no config provided', () => {
        ...
        assert.equal(goodNodes[0].attributes.key, 'entrypoint-app-good-autonamed', 'there was no mapping for this web-resource, so the key is auto-generated');
    });
    ...
    it('should auto-generate the name when the supplied value is not a string', () => {
        ...
        assert.equal(badNodes[0].attributes.key, 'entrypoint-app-bad-objectlike', 'an object is not a string, so the key is auto-generated');
        assert.equal(badNodes[1].attributes.key, 'entrypoint-app-bad-falsy', 'falsy values are not strings, so the key is auto-generated');
    });
});
